...
***********************************************************************/
func main() {
	c, err := redis.Dial("tcp", "47.104.66.33:6379")
	if err != nil {
	...
	defer c.Close()
	//////////////////////////////////////////////////////////////
	//_, err1 := c.Do("zadd", "curbike", nil, nil)
	_, err1 := c.Do("zadd", "curbike", 101, "mobike", 100, "xiaolan", 103, "ofo", 104, "xiaoming")
	_, err2 := c.Do("zadd", "tmpdata", 0, "mobike", 0, "xiaolan", 0, "mysql", 0, "redis", 0, "mongo", 0, "xiaoming")
	if err1 != nil || err2 != nil {
		fmt.Println("zadd failed", err.Error())
	}

	num, err := redis.Values(c.Do("zincrby", "curbike", 5, "xiaoxin"))
	...
	//res, err := redis.Values(c.Do("zrange", "curbike", -1, -1, "withscores"))
	res, err := redis.Values(c.Do("zrange", "curbike", -2, -1, "withscores"))
	var maxnum int64
	...
	if err != nil {
		...
		for _, v := range res {
			fmt.Printf("vvv:%s\n", v.([]byte))
			//s := Strings.Split(v.([]byte), []byte(" "))

			//s:= strings.Split(string(v.([]byte)), " ")
			//fmt.Println("s", s)
			//if len(s) == 2 {

			maxnum, _ = strconv.ParseInt(string(v.([]byte)), 10, 64)
	...
	res, err = redis.Values(c.Do("zrange", "curbike", -7, -1, "withscores"))
	...
	_, err = c.Do("zremrangebyscore", "curbike", 0, maxnum)
	...
	c.Do("hmset", "runoobkey", "child1", "parent1")
	c.Do("hmset", "runoobkey", "child2", "parent1")
	c.Do("hmset", "runoobkey", "child3", "parent2")

	res, err = redis.Values(c.Do("hgetall", "runoobkey"))
	if err != nil {
		fmt.Println("zincrby failed", err.Error())
	} else {
		for _, v := range res {
	...
	ok,err:=  c.Do("hexists", "runoobkey", "child4")
	...
	res, err = redis.Values(c.Do("hkeys", "runoobkey"))
	if err != nil {
		fmt.Println("zincrby failed", err.Error())
	} else {
		for _, v := range res {
			fmt.Printf("key:%s\n", v.([]byte))
			c.Do("hdel", "runoobkey",v)
		}
	...
	//}
	//////////////////////////////////////////////////////////////////
	//num, err := redis.Values(c.Do("zincrby", "curbike", 1, "xiaolan"))
	//fmt.Println(reflect.TypeOf(num))
	//if err != nil {
	//	fmt.Println("zincrby failed", err.Error())
	//} else {
	...
	//}
	////////////////////////////////////////////////////////////////////////////////
	//res, err = redis.Values(c.Do("zrangebyscore", "curbike", "(0", "(6"))
	//if err != nil {
